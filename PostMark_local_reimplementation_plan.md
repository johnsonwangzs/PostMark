...
6. 不依赖或重建缺失的 `paragram_xxl.pkl`；检测固定使用本地 Nomic fuzzy，exact-lemma 仅作为消融。
...
- 大规模并行生成；
- 复杂攻击鲁棒性评估。
- Paragram 向量、兼容表和 `paragram_fuzzy` detector。
...
本方案中的“完全离线”指：模型、tokenizer 和 spaCy 数据完成离线部署后，资源构建、watermark、detect、test 全流程不发起网络请求。模型和语料的预先获取不属于运行阶段，但必须在文档中单独说明。
...
### 13.1 原论文 detector 差异（仅作说明）

论文使用的 detector 资源和完整实现无法取得。本项目不构建 Paragram
资源，也不提供对应运行模式；该差异必须在实验报告中披露。因此所有
检测结果均标记 `paper_method_compatible=false` 和
...
### 13.3 Presence 模式 C：`nomic_fuzzy`（portable 主检测器）
...
- 结果必须标记为 portable，不得冒充论文原始 detector 数值。
- 运行时保持与 table 一致的 selector `implementation_profile`，并设置 `detector_profile=portable`。
...
- `detector_config_sha256` 覆盖 selection config hash、detector profile、presence mode、Nomic presence resource hash、similarity threshold、spaCy/tokenizer fingerprint、token filter 和 OOV 规则；
...
以下资源必须预先放到本地：LLM、Nomic model、对应 tokenizer、spaCy 英文模型和独立 anchor corpus。联网下载或制作 wheelhouse 属于部署准备，不属于正式运行流程。
...
- 网络隔离 smoke test 在两个 offline 环境变量开启时，从 tiny corpus 构建 candidate/Nomic 测试资源，再完成 watermark/detect，整个流程无网络访问。
...
- portable Nomic fuzzy keyed blind detector 与 exact-lemma 消融；
...
2. 新增文件列表；
3. 删除的在线依赖列表；
4. compat 候选词和 Nomic anchor table 构建命令；
5. watermark 命令；
6. Nomic fuzzy 主 detector、exact-lemma 消融和 calibration/held-out 评测命令；
7. 测试命令；
